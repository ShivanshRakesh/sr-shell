...
void show_prompt(void)
{
    ...
    if (git_branch)
    {
        spacing = m[1] - 7 - strlen(getenv("USER")) - strlen(pwd) - strlen(hostname) - strlen(git_branch);
        printf("\033[1;32m%s@%s:\033[0;36m %s \033[0mon \033[1;32m%s\033[0;32m%*s\n\033[1;36m> ", getenv("USER"), hostname, pwd, git_branch, spacing, PROMPT_RIGHT);
    }
    else
    {
        spacing = m[1] - 3 - strlen(getenv("USER")) - strlen(pwd) - strlen(hostname);
        printf("\033[1;32m%s@%s:\033[0;36m %s\033[0;32m%*s\n\033[1;36m> ", getenv("USER"), hostname, pwd, spacing, PROMPT_RIGHT);
    }
    printf("\033[0m");
}